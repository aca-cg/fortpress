...
/*
	...
	Just put type="socket.io" in a srv.conf.js file
...
*/
var wf = WF();
wf.AppServer['socket.io'] = SocketIO;
...
function SocketIO(srv)
{
    ...
    function httpHandler(req, res)
    {
        ...
        if(wf.SERVERS[srv].HOSTMAP[req.host])
        {
			...
			{
				if(!req.HOST.appArray[i].init.type || req.HOST.appArray[i].init.type.indexOf('http') > -1)
				{
    ...
    socket.CLIENT = CLIENT;
        ...
        if(wf.SERVERS[srv].HOSTMAP[socket.host])
        {
            ...
            for(var i = 0; i < appL; i++)
            {
              if(socket.HOST.appArray[i].init.type && socket.HOST.appArray[i].init.type.indexOf('socket.io') > -1)
              {
